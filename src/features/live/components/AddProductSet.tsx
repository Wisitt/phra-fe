...
import AddEditProductSetDialog from './AddEditProductSetDialog'
import MyProductSetCard from './MyProductSetCard'

export default function AddProductSet() {
  ...
  const handleDelete = (index: number) => {
    setProductSets(prev => prev.filter((_, i) => i !== index))
  }
  ...
  const handleSubmit = (values: ProductSet) => {
    if (editIndex !== undefined) {
      setProductSets(prev => prev.map((productSet, i) =>
        i === editIndex ? values : productSet,
      ))
    }
    else {
      setProductSets(prev => [...prev, values])
    }
  }
  ...
  return (
    <div className="grid grid-cols-5 gap-4">
      ...
      <button
        type="button"
        onClick={handleAdd}
        className="flex h-[414px] cursor-pointer flex-col items-center justify-center gap-4 rounded-[20px] border border-dashed border-muted-foreground text-muted-foreground ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
      >
      ...
      <AddEditProductSetDialog
        isOpen={isDialogOpen}
        mode={dialogMode}
        setIsOpen={setIsDialogOpen}
        initialValues={editIndex !== undefined ? productSets[editIndex] : undefined}
