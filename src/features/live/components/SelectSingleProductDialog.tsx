...
import { generateMockData } from '@/features/live/api/live'
...
interface Props {
  ...
  selectedItems: (Product | Auction | Art)[]
  onSelectionChange: (updatedSelectedItems: (Product | Auction | Art)[]) => void
}

export default function SelectSingleProductDialog({
  isOpen,
  setIsOpen,
  selectedItems,
  onSelectionChange,
}: Props) {
  const [allItems] = useState<(Product | Auction | Art)[]>(generateMockData())
  const [tempSelectedItems, setTempSelectedItems] = useState<(Product | Auction | Art)[]>(selectedItems)

  const toggleSelection = (itemId: number) => {
    const isSelected = tempSelectedItems.some(item => item.id === itemId)
    const updatedSelectedItems = isSelected
      ? tempSelectedItems.filter(item => item.id !== itemId)
      : [...tempSelectedItems, allItems.find(item => item.id === itemId)!]

    setTempSelectedItems(updatedSelectedItems)
  }

  const renderItemCard = (
    item: Product | Auction | Art,
    isSelected: boolean,
    handleToggle: (itemId: number) => void,
  ) => {
    const commonProps = { isSelected, onSelect: () => handleToggle(item.id) }

    switch (item.productTypeId) {
      case 1:
        return (
          <MyProductCard key={item.id} product={item as Product} {...commonProps} />
        )
      case 2:
        return (
          <MyAuctionCard key={item.id} auction={item as Auction} {...commonProps} />
        )
      case 3:
        return <MyArtCard key={item.id} art={item as Art} {...commonProps} />
      default:
        return null
    }
  }
  ...
  const confirmSelection = () => {
    onSelectionChange(tempSelectedItems) // Update the parent state
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="max-w-[1180px]">
        ...
        <DialogBody>
          <Accordion type="single" collapsible defaultValue="item-1">
            <AccordionItem value="item-1">
              <AccordionTrigger>รายการสินค้าแบบเดี่ยว</AccordionTrigger>
              <AccordionContent enableOverflow className="grid grid-cols-4 gap-4">
                {allItems.map(item =>
                  renderItemCard(
                    item,
                ...
                <Link
                  href="/new"
                  className="flex h-full min-h-[394px] cursor-pointer flex-col items-center justify-center gap-4 rounded-[20px] border border-dashed border-muted-foreground text-muted-foreground ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                >
        ...
        <DialogFooter>
          <Button onClick={confirmSelection} className="rounded-full sm:w-[120px]">
            ยืนยัน
          </Button>
          <DialogClose asChild>
            <Button variant="outline" className="rounded-full sm:w-[120px]">
              ยกเลิก
            </Button>
          </DialogClose>
        </DialogFooter>
